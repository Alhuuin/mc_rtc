...
void TorqueCartesianTask::update(mc_solver::QPSolver & solver)
{
  ...
  if(integralTermEnabled_)
  {
    ...
    forceComponents += integralWrench_.vector();
  }

  forceComponents += WrenchTask::cartesianInertia() * stiffness_.cwiseProduct(posError_.vector());
  forceComponents += WrenchTask::cartesianInertia() * damping_.cwiseProduct(velError_.vector());
  ...
  if(compensateGravity_) { forceComponents += wrenchGravityCompensation_.vector(); }

  forceComponents += WrenchTask::dynamicJacobianTranspose() * torqueComponents;

  wrenchTarget_ = sva::ForceVecd(forceComponents.head<3>(), forceComponents.tail<3>());
...
void TorqueCartesianTask::computeForceGravityCompensation()
{
  ...
  Eigen::VectorXd Cg = robot.tvmRobot().C();
